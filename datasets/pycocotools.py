...
class COCO:
    def __init__(self, args, cls_order, phase_idx, incremental, incremental_val, val_each_phase, balanced_ft, tfs_or_tfh, num_of_phases, cls_per_phase, seed_data, annotation_file=None):
        """
        ...
        if balanced_ft:
            ...
            for phase_idx in range(total_phase_num):
                ...
                if not incremental_val:
                    ...
                    num_img_this_phase = int(total_num_img_dataset)

                np.random.seed(seed_data)
                data_order = np.arange(total_num_img_dataset)
                np.random.shuffle(data_order)

                if not incremental_val:
                    if tfs_or_tfh == 'tfs':
                        selected_img_idx_array = data_order[phase_idx*num_img_this_phase:(phase_idx+1)*num_img_this_phase]
                    elif tfs_or_tfh == 'tfh':
                        if phase_idx == 0:
                            selected_img_idx_array = data_order[0:num_img_this_phase]
                        else:
                            selected_img_idx_array = data_order[half_dataset+(phase_idx-1)*num_img_this_phase:half_dataset+(phase_idx)*num_img_this_phase]
                    else:
                        raise ValueError('Please set the correct data setting.')
                    
                    selected_img_idx_array = selected_img_idx_array[0:int(len(selected_img_idx_array)*args.mem_rate)]
                else:
                    selected_img_idx_array = data_order
                ...
                if args.debug_mode:
                    ...
                    if 'images' in self.dataset:
                        for img in self.dataset['images'][0:10]:
                            imgs[img['id']] = img
                    ...
                    if incremental:
                        if incremental_val:
                            ...
                            this_phase_labaled_img_id_set = set(this_phase_labaled_img_id_list)
                            ...
                            this_phase_imgs = {}
                            this_phase_img_id_list = []
                            for img_idx in selected_img_idx_array:
                                img = self.dataset['images'][img_idx]
                                this_phase_imgs[img['id']] = img
                                this_phase_img_id_list.append(img['id'])
            ...
            if args.debug_mode:
                ...
                if incremental:
                    if incremental_val:
                        ...
                        if 'categories' in self.dataset:
                            for cat in self.dataset['categories']:
                                if cat in selected_cls_this_phase:
                                    cats[cat['id']] = cat
                    else:
                        this_phase_imgs = {}
                        this_phase_img_id_list = []
